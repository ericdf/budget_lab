...
import { formatMoney, formatPct, CATEGORY_LABELS } from '../utils/calculations'
...
export default function ImpactPanel() {
  ...
  // Max bar width reference (the full deficit)
      ...
      <div className="space-y-3 sticky top-[88px]">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          ...
          <div className="text-center mb-3">
            ...
            <p className="text-xs text-gray-500">of deficit closed</p>
          </div>
          ...
          {activeCount === 0 ? (
            ...
            <div className="space-y-2.5">
              <TimingBar
                ...
                width={nowW}
                amount={helps_now}
                hint="Permanent or immediate impact"
              />
              <TimingBar
                label="Helps later"
                color="bg-blue-500"
                ...
                width={laterW}
                amount={helps_later}
                hint="Savings materialize in 1–3 years"
              />
              <TimingBar
                label="Pushes cost forward"
                color="bg-orange-400"
                textColor="text-orange-700"
                width={fwdW}
                amount={pushes_forward}
                hint="Makes future budgets harder"
              />
            </div>
          )}
        </div>
              ...
              <div className="flex h-4 rounded-full overflow-hidden gap-px mb-3">
                {structural_share > 0 && (
                  <div
                    ...
                    style={{ width: `${structural_share * 100}%` }}
                    title={`Permanent/structural: ${Math.round(structural_share * 100)}%`}
                  />
                )}
                {delayed_share > 0 && (
                  <div
                    className="bg-blue-500 transition-all"
                    style={{ width: `${delayed_share * 100}%` }}
                    title={`Delayed: ${Math.round(delayed_share * 100)}%`}
                  />
                )}
                {temporary_share > 0 && (
                  <div
                    className="bg-orange-400 transition-all"
                    style={{ width: `${temporary_share * 100}%` }}
                    title={`Temporary: ${Math.round(temporary_share * 100)}%`}
                  />
                )}
              </div>
